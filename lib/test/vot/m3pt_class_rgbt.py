...
from lib.test.vot.vot20_utils import *
...
def read_rgbt_img(color_path, tir_path):
    if color_path:
        ...
        rgb = None
    if tir_path:
        tir = cv2.imread(tir_path, -1)
    else:
        tir = None
    tir = np.asarray(tir, dtype=np.uint8)
    colormap = cv2.applyColorMap(tir, cv2.COLORMAP_JET)
...
def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='color'):
    ...
    if isinstance(imagefile, list) and len(imagefile)==2:
        image = decode_img(imagefile[0], imagefile[1])
    else:
