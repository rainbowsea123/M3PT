...
class VTUAV(BaseVideoDataset):
    def __init__(self, root=None, split='train', dtype='rgbrgb', seq_ids=None, data_fraction=None):
        """
        args:
            root - path to the VTUAV trainingset.
        ...
        assert split in ['train', 'val', 'all'], 'Only support all, train or val split in VTUAV, got {}'.format(split)
    ...
    def _get_sequence_list(self, split):
        ...
        file_path = os.path.join(ltr_path, 'data_specs', 'vtuav_{}.txt'.format(split))
        with open(file_path, 'r') as f:
            dir_list = f.read().splitlines()
        return dir_list

    def _read_bb_anno(self, seq_path):
        # in vtuav dataset, rgb.txt is different from ir.txt
        rgb_bb_anno_file = os.path.join(seq_path, "rgb.txt")
        # ir_bb_anno_file = os.path.join(seq_path, "ir.txt")
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        rgb_frame_path = os.path.join(seq_path, 'rgb', '{:06d}.jpg'.format(frame_id))  # frames start from 0
        ir_frame_path = os.path.join(seq_path, 'ir', '{:06d}.jpg'.format(frame_id))
