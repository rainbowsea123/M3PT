...
def genConfig(seq_path, set_type):
    if set_type == 'RGBT234':
        ...
        ############################################  have to refine #############################################
        RGB_img_list = sorted([seq_path + '/v/' + p for p in os.listdir(seq_path + '/v') if os.path.splitext(p)[1] == '.png' or os.path.splitext(p)[1] == '.bmp'])
        T_img_list = sorted([seq_path + '/i/' + p for p in os.listdir(seq_path + '/i') if os.path.splitext(p)[1] == '.png'  or os.path.splitext(p)[1] == '.bmp'])
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    if script_name == 'm3pt':
        params = rgbt_prompt_params.parameters(yaml_name, epoch)    # there to modify the ckp load path
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GTOT':
        seq_home = '/media/ubuntu/data/wqm/data/GTOT'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home, f))]
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/media/ubuntu/data/wqm/data/RGBT234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT210':
        seq_home = '/media/ubuntu/data/wqm/data/RGBT210'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/media/ubuntu/data/wqm/data/lasher/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'VTUAVST':
        seq_home = '/media/ubuntu/data/wqm/data/VTUAV/test_ST'
        with open(join(join(seq_home, 'VTUAV-ST.txt')), 'r') as f:
            seq_list = f.read().splitlines()
    elif dataset_name == 'VTUAVLT':
        seq_home = '/media/ubuntu/data/wqm/data/VTUAV/test_LT'
